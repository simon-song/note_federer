...
\begin{proof}
\begin{enumerate}
  \item For $i=1,2,\cdots$, let $C_i$ be a $\phi$ measurable set satisfying 
    $A_i\subset C_i$ and $\phi(C_i)=\phi(A_i)$. Let
    $B_i=\bigcap_{j=i}^{\infty} C_j$, then $B_i$ is $\phi$ measurable, and 
    $B_1\subset B_2\subset B_3\subset\cdots$ is an increasing sequence. 
    ...
    \[
      \phi\left( \bigcup_{i=1}^{\infty} A_i \right)
        ...
        = \lim_{i\to\infty} \phi(B_i)
        = \lim_{i\to\infty} \phi(A_i).
    \]
    From the monotonity of measure we also have 
    \[
      \phi\left( \bigcup_{i=1}^{\infty} A_i\right) 
...
for any sequence of sets $(A_i)$ we have
\begin{align*}
...
An alternative proof for the countable subadditivity is the following.
\footnote{cf. Cohn, Measure Theory, 2nd ed., Proposition 1.5.4, p.33}
% First note that for any set $A$ there exists a $\phi$-measurable 
% set $B$ such that $A\subset B$ and $\gamma(A)=\phi(B)$. 
% Now $\bigcup_i B_i$ is also 
% a $\phi$-measurable set, hence $\gamma(\bigcup_i B_i)=\phi(\bigcup_i B_i)$ by 
% defintion of $\gamma$, and
% \[
%   \gamma(\bigcup_i A_i) \le \gamma(\bigcup_i B_i) 
%     = \phi(\bigcup_i B_i) \le \sum_i \phi(B_i) = \sum_i \gamma(A_i)
% \]
Fix $\epsilon>0$, for any set $A_i$ there exists a $\phi$-measurable set $B_i$ 
such that $A_i\subset B_i$ and $\phi(B_i)\le \gamma(A_i)+\epsilon 2^{-i}$. 
Now $\bigcup_i B_i$ is also a $\phi$-measurable set, hence 
$\gamma(\bigcup_i B_i)=\phi(\bigcup_i B_i)$ by definition of $\gamma$. Thus
\[
  \gamma(\bigcup_i A_i) \le \gamma(\bigcup_i B_i) 
    = \phi(\bigcup_i B_i) \le \sum_i \phi(B_i) 
    \le \sum_i \gamma(A_i) + \epsilon.
\]
Since $\epsilon$ is arbitrary, we conclude that $\gamma$ is countable
subadditive. 
...
\footnote{Note also that $B$ satisfies $\gamma(A)=\phi(B)$, i.e. the infimum
in the defintion of $\gamma$ is attained.}
